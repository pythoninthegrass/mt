...
async def get_tracks(
    ...
    album: str | None = None,
    genre: str | None = None,
    sort: str = Query("title", enum=["title", "artist", "album", "duration", "date_added", "play_count"]),
...
@router.get("/tracks-table", response_class=HTMLResponse)
async def get_tracks_table(
    request: Request,
    skip: int = Query(0, ge=0),
    limit: int = Query(100, ge=1, le=1000),
    search: str | None = None,
    ...
    """Get tracks as HTML table rows for HTMX."""
    query = select(Track)
...
async def scan_library(
    ...
    # If no path provided, return instruction to open dialog
    if path is None:
        return {
            "message": "Please select a directory to scan",
            "action_required": "open_directory_dialog",
    ...
    if not scan_path.exists() or not scan_path.is_dir():
        ...
        raise HTTPException(status_code=400, detail=f"Invalid directory path: {path}")

    # Start scan in background
    background_tasks.add_task(library_service.scan_directory, path)

    return {"message": "Library scan initiated", "path": path, "status": "scanning"}
...
async def get_artists(
    ...
    query = select(Track.artist, func.count(Track.id).label("track_count")).where(Track.artist.isnot(None)).group_by(Track.artist)
    ...
    return [{"artist": artist, "track_count": count} for artist, count in artists]
...
async def get_albums(
    ...
    query = (
        select(Track.album, Track.artist, Track.year, func.count(Track.id).label("track_count"))
        .where(Track.album.isnot(None))
        .group_by(Track.album, Track.artist, Track.year)
    )
    ...
    return [{"album": album, "artist": artist, "year": year, "track_count": count} for album, artist, year, count in albums]
