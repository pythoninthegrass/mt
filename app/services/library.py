...
class LibraryService:
    ...
    async def scan_directory(self, path: str | None = None):
        """Scan a directory for music files using Zig scanner."""
        scan_path = Path(path) if path else settings.MUSIC_LIBRARY_PATH

        if not scan_path.exists():
            await broadcast_library_event(EventTypes.ERROR, {"message": f"Path does not exist: {scan_path}"})
            return

        await broadcast_library_event(EventTypes.LIBRARY_SCAN_STARTED, {"path": str(scan_path)})

        # Initialize Zig scanner
        scanner = ZigScannerAsync()

        # Discover all music files using Zig scanner
        music_files = await scanner.discover_files(str(scan_path), return_list=True)
        ...
        for i in range(0, total_files, chunk_size):
            ...
            for file_path in chunk:
                try:
                    track = await self.process_music_file(Path(file_path))
                    if track:
                        new_tracks += 1
                ...
                if processed_files % 10 == 0:
                    await broadcast_library_event(
                        EventTypes.LIBRARY_SCAN_PROGRESS,
                        {
        ...
        await broadcast_library_event(
            EventTypes.LIBRARY_SCAN_COMPLETED,
            {"total_files": total_files, "processed_files": processed_files, "new_tracks": new_tracks},
        )
