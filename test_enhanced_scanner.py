...
async def test_basic_scan():
    ...
    async def progress_callback(progress):
        print(f"Progress: {progress.percentage:.1f}% - {progress.processed_files} files "
...
async def test_file_discovery():
    ...
    # Test with file list (limit to first 10 for display)
    ...
    class MockWebSocket:
        async def send_json(self, data):
            ...
            if msg_type == 'scan_progress':
                progress = data['data']
                print(f"[WS] Progress: {progress['percentage']:.1f}% - "
                      f"{progress['current_path']}")
